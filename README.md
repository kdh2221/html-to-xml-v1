...
> **지금 상태**: 자동 검사(테스트) **233개 모두 통과**, 실수 0개. 예시 화면 3개(간단폼·검색표·마스터디테일)로 매번 확인하고 있어요.
...
Phase 1 룰 ID prefix UI-01 + 버튼 modifier                    [✅ Phase 0+1]
 ...
 ▼
Stage 4   scwin 조회 핸들러 (onpageload/onclick/submitdone)   [✅ Phase 2C-1]
 │
 ▼
(예정)    저장 흐름(2C-3) → 안티패턴 검증(3) → 시각 회귀(4)    [⏳ 미구현]
 ▼
최종 WebSquare XML
```
...
# 테스트 (전체 233 PASS, 1 live-llm skip)
...
- 2A: LLM 데이터 추론 / 2B: 바인딩+제출 / 2C-0: schbox 정규화 / 2C-1: 조회 동작 / 2C-2: 상세영역 연결 / 2C-3: 저장 흐름+검증
