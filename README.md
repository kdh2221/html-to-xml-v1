...
```
   ...
   [5단계] 버튼 동작(조회 누르면 실행) 적어 넣기 ★2C-1
      │
   [곁들임] 원본 그림의 CSS·JS를 "참고용 메모"로 따로 챙겨 두기 ★원본보존
      │
   완성된 WebSquare 화면 ✅
```
...
| **원본 CSS·JS 챙겨두기** | 원본 보존 | 변환할 때 버려지던 원본의 **스타일(CSS)과 동작(JS)을 "참고용 메모"로 따로 저장**해요. (`화면.source.css` / `화면.source.js` 파일로 옆에 챙겨 두고, 완성된 XML 맨 위에 "원본 메모 여기 있어요" 쪽지를 붙여 둡니다.) 자동으로 적용하진 않고, 나중에 사람이 손볼 때 **참고 자료**로 씁니다. | ✅ 완료 |

> **지금 상태**: 자동 검사(테스트) **288개 모두 통과**, 실수 0개. 예시 화면 3개(간단폼·검색표·마스터디테일)는 품질 점검 **위반 0건** + 변환 **보존율 100%**로 깨끗합니다.
...
| **사이드카 (.source.css/.js)** | 변환 결과 XML 옆에 같이 챙겨 두는 원본 스타일·동작 "참고용 메모" 파일 (자동 적용은 안 함) |
...
검증      9개 안티패턴 정적 검사 (onStage/CLI 리포트, 비파괴)  [✅ Phase 3A]
 ...
 │        + 원본 CSS/JS 사이드카 (onStage만, 출력 불변)        [✅ 원본 보존]
 ▼
...
# 테스트 (전체 288 PASS, 1 live-llm skip)
...
│       │   ├── source-assets.ts         # 원본 CSS/JS 추출 + XML 참조 주석 (참조 전용)
│       │   ├── pipeline.ts              # 전체 오케스트레이터
│       │   ├── cli.ts                   # CLI 엔트리 (+ .source.css/.js 사이드카 기록)
...
| 원본 CSS/JS 보존 | `src/source-assets.ts` (파이프라인 onStage + CLI 사이드카·주석) |
...
- 2A: LLM 데이터 추론 / 2B: 바인딩+제출 / 2C-0: schbox 정규화 / 2C-1: 조회 동작 / 2C-2: 상세영역 연결 / 2C-3: 저장 흐름+검증 / 3A: 안티패턴 검증+해결 / 4: 변환 보존 리포트 / 원본 보존: `2026-05-22-source-asset-preservation-design.md`
