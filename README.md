...
> **지금 상태**: 자동 검사(테스트) **264개 모두 통과**, 실수 0개. 예시 화면 3개(간단폼·검색표·마스터디테일)는 품질 점검에서 **위반 0건**으로 깨끗합니다.
...
검증      9개 안티패턴 정적 검사 (onStage/CLI 리포트, 비파괴)  [✅ Phase 3A]
 │
 ▼
(예정)    더 많은 자동수정(3B) → LLM 피드백(3C) → 시각 회귀(4)  [⏳ 미구현]
 ▼
최종 WebSquare XML
```
...
# 테스트 (전체 264 PASS, 1 live-llm skip)
...
- 2A: LLM 데이터 추론 / 2B: 바인딩+제출 / 2C-0: schbox 정규화 / 2C-1: 조회 동작 / 2C-2: 상세영역 연결 / 2C-3: 저장 흐름+검증 / 3A: 안티패턴 검증+해결
